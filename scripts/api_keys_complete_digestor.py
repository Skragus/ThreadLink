...
# API key related file patterns - both source and test files
api_key_files = [
...
def clean_file_content(content, file_path):
    ...
    content = re.sub(
        r"\/\*[\s\S]*?\*\/", "", content
    )  # Strip single-line comments (but keep them in test files for context)
    ...
    )  # Strip exports (but not in test files)
    if "test" not in file_path.lower():
        content = re.sub(
...
output_file = f"quickmap_api_keys_complete_{timestamp}.md"
with open(output_file, "w", encoding="utf-8") as f:
    f.write("\n\n" + "=" * 80 + "\n\n".join(all_content))

print(f"Complete API Keys digest created: {output_file}")
